              ...
              <template slot-scope="scope">
                <div class="tab-name">
                  ...
                  <span class="code">
										...
										<i v-else-if="scope.row.stock_type === 'sh'" class="iconfont hu-stock">沪</i>
										<i v-else-if="scope.row.stock_type === 'sz'" class="iconfont shen-stock">深</i>
                    <i v-else-if="scope.row.stock_type === 'hk'" class="iconfont shen-stock">港</i>
		...
		methods: {
          ...
          opt = {
            ...
            pageSize: this.form.pageSize * this.form.pageNum,
				...
				let data = null;
				if (this.activeName === "first" || this.activeName === "four" || this.activeName == 'start' || this.activeName === "gangu" || this.activeName == 'meigu') {
          ...
          opt = {
            ...
            pageSize: this.form.pageSize,
				...
				this.loading = true;
				let data = null;
				if (this.activeName === "first" || this.activeName === "four" || this.activeName === "gangu" || this.activeName === "meigu" ) {
					data = await api.getStock(opt);
				} else if (this.activeName === 'start') {
					data = await api.getStock(opt);
				} else {
				...
				if (data.status === 0) {
					...
					// });
					this.list = data.data;
          //美股港股console.warn(data.data.list[0]);
					// 出仓数据
					var res = await api.findUserPositionByCode({
						stockCode: data.data.list[0].gid,
					});
					if (res.status == 0) {
			...
			async toTransaction(row, column, event) {
				...
				// }
				let code =
					this.activeName === "first" || this.activeName === "four" || this.activeName == 'start' || this.activeName == 'gangu' || this.activeName == 'meigu'  ?
					row.code :
					row.stockCode;
